...
public interface ICrmLeadService
{
    ...
    Task<CrmLead> CreateRecommendationRequestAsync(string contactName, string contactPhone, string? contactEmail, DateOnly? desiredCheckInDate, DateOnly? desiredCheckOutDate, int? guestCount, string? notes, CancellationToken cancellationToken = default);
